...
class BlowFish(object):
    def __init__(self, key, bitlength = 448):
        ...
        for i in range(0,17,2):
            ...
            self.parray[i], self.parray[i+1] = output>>32, output & 0xFFFFFFFFL

        # re-encrypt and reassign through all the S-boxes
        for i in range(4):
            for j in range(0,255,2):
                output = self.bfencrypt(output)
                self.sbox[i][j],self.sbox[i][j+1] = output>>32, output & 0xFFFFFFFFL
    ...
    def F(self, xL):
        ...
        # apply function
        rtnval = (self.sbox[0][qs[0]]+self.sbox[1][qs[1]])%(1L<<32) # add S1+S2
        rtnval ^= self.sbox[2][qs[2]]                          # XOR w/ S3
        return (rtnval+self.sbox[3][qs[3]])%(1L<<32)           # add S4
