...
def get_arg_types(app):
    ...
    if positional is None:
        ...
        arg_positions = {
            argname: idx for idx, argname in enumerate(m.args[1:])
        }
        ...
        num_args = len(positional)
    return num_args, [
        getattr(argclass, '__name__', None) for argclass in positional
    ]
...
def get_app_info(app):
    ...
    for name, optinfo in app._switches_by_name.items():
        if len(name) == 1:  # single letter option, ignore
            continue
        name = "--" + name
        app_info['options'][name] = {
            'description': optinfo.help
        }
...
def dump_assign_vars(assign_vars):
    indented_cr = '\n' + (' ' * 12)
    assignments = tuple(
        f'{name}={value}' for name, value in assign_vars
    )
...
def dump_app_section(path, app_tree):
    ...
    if len(subapps) > 0:
        ...
        assign_vars += [ ('subapps', quoted(' '.join(subapps))),
                         ('described_subapps', '"\\\n' + described_subapps + '"') ]
    ...
    if len(options) > 1:
        described_options = get_described_items(options,
                get_option_fullname)
        assign_vars += [ ('options', quoted(' '.join(options))),
                         ('described_options', '"\\\n' + described_options + '"') ]
        optargnames = { optname: optinfo['argname'] \
    ...
    print(APP_SECTION % dict(
        path = ' '.join(path),
        assign_vars = dump_assign_vars(assign_vars)
    ), end='')
