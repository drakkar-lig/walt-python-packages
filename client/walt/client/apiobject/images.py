...
class APIImageInfoCache(APIItemInfoCache):
    ...
    def do_refresh(self, server):
        ...
        image_infos = list({ f: val for f, val in zip(fields, row) } for row in tabular_data)
        # populate attributes
        self.info_per_id = {}
        self.id_per_name = {}
...
class APIImageFactory:
    ...
    def create(in_image_name):
        ...
        if api_image is not None:
            return api_image
        item_cls = APIItemClassFactory.create(__info_cache__, in_image_name,
                                        'image', APIImageBase, APISetOfImagesFactory)
        class APIImage(item_cls, APIImageBase):
            ...
            def rename(self, new_name):
                ...
                APIImageFactory.__images_per_name__[new_name] = APIImageFactory.__images_per_name__.pop(name)
        api_image = APIImage()
...
class APISetOfImagesFactory:
    ...
    def create(item_set_factory, in_names):
        item_set_cls = APISetOfItemsClassFactory.create(
            __info_cache__, in_names, 'image', APIImageBase,
            APIImageFactory, APISetOfImagesBase, item_set_factory)
...
class APIImagesSubModule(APIObjectBase):
    ...
    def build(self, image_name, dir_or_url):
        ...
        if mode == 'dir':
            if not Path(dir_or_url).is_dir():
                sys.stderr.write('Failed: parameter dir_or_url must be a directory or a git repository URL.\n')
                return
    ...
    def clone(self, clonable_image_link, force=False, image_name=None):
        ...
        with silent_server_link() as server:
            res = server.clone_image(clonable_image_link,
                                    force=force, image_name=image_name)
            if res['status'] == 'OK':
                __info_cache__.refresh()    # detect the new image
...
class APIClonableImage(APIObjectBase):
    def clone(self, force=False, image_name=None):
        ...
        return images_submodule.clone(self.clonable_link,
                        force=force, image_name=image_name)
