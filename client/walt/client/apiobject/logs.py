...
class APILogsSubModule(APIObjectBase):
    ...
    def get_logs(self, realtime=False, history=None, issuers='my-nodes', timeout=-1):
        ...
        with silent_server_link() as server:
            if history is not None:
                range_analysis = WalTLogShowOrWait.analyse_history_range(server, history)
                if not range_analysis[0]:
                    raise Exception('''Invalid "history" value. See 'walt help show log-history'.''')
                history_range = range_analysis[1]
            else:
                history_range = None
            issuers = get_devices_names(server, issuers, allowed_device_set='server,all-nodes')
            if issuers is None:
    ...
    def _iterate(self, history_range, realtime, issuers, timeout):
        ...
        nt_cls = None
        it = WalTLogShowOrWait.start_streaming(history_range, realtime,
                issuers, None, None, None, timeout)
        for record in it:
