    ...
    for attrs, props in ((ro_attrs, ro_props), (rw_attrs, rw_props)):
        for prop in props:
            try:
                ...
                attrs[prop] = e
    desc = ''
    for category, attrs in (('read-only attributes', ro_attrs),
                            ('writable attributes', rw_attrs)):
        if len(attrs) > 0:
            desc += f'\n  {category}:\n' + '\n'.join(
                f'  - self.{k}: {short_repr(v)}' \
...
def get_subitems_desc(obj):
    ...
    if len(items) == 0:
        return ''
    subitems_text = '\n'.join(
        f'  - {get_subitem_accessor(k)}: {short_repr(v)}' \
                 for k, v in items)
    return f'''
  sub-items:
{subitems_text}
...
class APIItemClassFactory:
    @staticmethod
    def create(object_cache, in_item_name, item_cls_label, item_base_cls, item_set_factory):
        class APIItem(APIObjectBase):
            ...
            def __or__(self, other):
                if isinstance(other, item_base_cls):
                    return item_set_factory.create({self.__class__._item_name, other._item_name})
