...
class Pager:
    ...
    def parse_topic_links(self, ast):
        ...
        while event is not None:
            node = event['node']
            if node.t == 'link' and event['entering'] and \
                    node.destination.endswith(".md"):
        ...
        while True:
            ...
            if req == 'q':
                ...
                if len(topic_links) == 0:
                    continue
                next_selected_link_num = (selected_link_num + 1) \
                                % (len(topic_links))
                # ensure the viewport includes the selected link
                selected_link_line = topic_links[next_selected_link_num][1]
                if index + 2 > selected_link_line or index < selected_link_line - page_height + 2:
                ...
                if index > selected_link_line or index < selected_link_line - page_height:
