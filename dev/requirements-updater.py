...
def parse_rfc2822_part(text):
    ...
    return {
        k: v.strip()
        for k, v in (line.split(':', maxsplit=1) for line in textlines)
    }
...
def get_packages_info(package_names):
    ...
    proc = subprocess.run(cmd, text=True, stdout=subprocess.PIPE)
    return {
        p_info['Name'].lower(): p_info
        for p_info in parse_rfc2822(proc.stdout)
    }


def get_dependencies(package_name,
                     packages_info,
                     add_current_package=False):
    dependencies = {}
    ...
    if len(package_info['Requires'].strip()) > 0:
        ...
        for required_package_name in required_package_names:
            dependencies.update(
                get_dependencies(required_package_name,
                                 packages_info,
                                 add_current_package=True))
    return dependencies
...
if __name__ == '__main__':
    ...
    if mode == 'freeze':
        ...
        filepath.write_text(
            ...
            '\n'.join(
                (f'{k}' if k in PACKAGES_WITH_FLEXIBLE_VERSION
                 else f'{k}=={v}')
