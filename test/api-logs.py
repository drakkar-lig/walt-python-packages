...
def test_api_logs_get_logs():
    ...
    for logline in api.logs.get_logs(history='-10s:', realtime=True, issuers='server', timeout=60):
    ...
    while True:
        try:
            for logline in api.logs.get_logs(realtime=True, issuers=node, timeout=1):
                assert time() - t0 < 30, f'{node.name} still sending logs long after boot: {logline.line}'
