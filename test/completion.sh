...
test_tabtab_complete "walt node config $node " "boot.delay=         boot.mode=          boot.retries=       boot.timeout=       cpu.cores=          disks=              expose=             kexec.allow=        mount.persist=      netsetup=           networks=           powersave.timeout=  ram="
EOF
