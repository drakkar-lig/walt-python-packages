...
def prepare():
    ...
    if not (PXE_PATH / "walt-x86-64-snponly.efi").exists():
        orig_path = this_dir / "walt-x86-64-snponly.efi.gz"
        with gzip.open(orig_path) as f:
            (PXE_PATH / "walt-x86-64-snponly.efi").write_bytes(f.read())
            ...
            for directive in old_status - status:
                ...
                if args[0] == "SYMLINK":
                    ...
                    if not mount_exists(mount_path):
                        ...
                        valid_status = False
                        break
                    # If the node still exists (we probably got here because
                    # its boot mode or image changed) then apply the grace time
                    # before unmounting this node-rw mount.
                    # Otherwise, proceed immediately.
                    node_mac = params[0]
                    node_still_exists = (node_mac in set(db_nodes.mac))
                    if node_still_exists:
                        ...
                        discard_grace_time_of_directives((directive,))
                        removed_mounts.append(args)
        if valid_status:
            # -- check new entries
