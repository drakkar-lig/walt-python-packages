...
from walt.server.tools import ttl_cache
...
def run():
    ...
    def serve_vpn_enroll():
        # for security reason, we disallow explicit ip specification
        # so we can ensure the request really comes from the node
        node_ip = requester_ip()
