...
from walt.server.const import HTTP_BOOT_SERVER_PUB_KEY
...
def _detect_https(host, port, dev_name, dev_port):
    with warnings.catch_warnings():
        ...
        try:
            if requests.get(f'https://{host}:{port}', verify=False):
                return True  # https selected
        except Exception:
            pass
        try:
            if requests.get(f'http://{host}:{port}'):
                return False  # http selected
        except requests.exceptions.ConnectionError:
            raise bottle.HTTPError(500,
                f"{dev_name} seems down or not responding "
                f"on port {dev_port}.")
    return False
...
def _serve_link(dev_name, dev_port):
    ...
    dev_type, dev_links = dev_info
    link_info = dev_links.get(dev_port)
    if link_info is None:
        return bottle.HTTPError(404, "This port is not exposed.")
    server_port, _ = link_info
    if _detect_https('127.0.0.1', server_port, dev_name, dev_port):
        proto = 'https'
    else:
        proto = 'http'
