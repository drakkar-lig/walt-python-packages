...
class DHCPPacket:
    ...
    def _encode_options(self) -> bytes:
        ...
        for code, value in self.options.items():
            ...
            # Work around a bug seen in an old intel uefi firmware
            # where the bootfile option length was ignored, thus
            # the following bytes (of next options) were appended
            # to the bootfile name.
            # We add a null byte which is what this firmware excepts
            # to end the string. Correct implementations will
            # interpret this null byte as a one-byte padding option
            # (code 0), which should not harm.
            if code == OPT_BOOTFILE:
                result += bytes([0])
...
class WalTDHCPdEngine:
    ...
    def apply_pxe_options(self, pkt: DHCPPacket, reply: DHCPPacket):
        ...
        # note:
        # when walt-x86-undionly.kpxe or walt-x86-64-snponly.efi is running
