...
EDITOR_TOP_MESSAGE = "VPN authentication keys management"


def list_valid_auth_keys(context, auth_keys):
    ...
    if len(valid_keys) == 0:
        ...
        if mask_forgotten.sum() > 0:
            ...
            print((auth_keys_existing_dev.certid + ": was used by " +
                   auth_keys_existing_dev.device_label + "\n").sum())
    input("Type <enter> to return to the menu.")


def list_revoked_auth_keys(context, auth_keys):
    mask_revoked = auth_keys.revoked.astype(bool)
    ...
    if len(revoked_keys) == 0:
        ...
        if mask_forgotten.sum() > 0:
            ...
            print((auth_keys_existing_dev.certid + ": was associated to " +
                   auth_keys_existing_dev.device_label + "\n").sum())
    input("Type <enter> to return to the menu.")


def revoke_key(context, auth_keys):
    while True:
        try:
...
def quit_screen(context, auth_keys):
    context.update(should_continue=False)


def compute_menu_options(auth_keys):
    mask_revoked = auth_keys.revoked.astype(bool)
...
def run():
    ...
    with alternate_screen_buffer():
        while True:
            ...
            print()
            menu_options = compute_menu_options(auth_keys)
            action = choose("Select a command", menu_options)
            print()
            action(context, auth_keys)
