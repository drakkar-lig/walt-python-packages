...
CLEANUP_DELETED_BATCH = 500      # max logs deleted per stream per batch
CLEANUP_DELETED_BUDGET = 0.03    # max time spent per cleanup step
CLEANUP_DELETED_DELAY = 0.03     # delay before the next cleanup step
...
class ServerDB(PostgresDB):
    ...
    def _create_tables(self):
        ...
        self.execute("""CREATE TABLE alllogs (
            stream_id INTEGER REFERENCES alllogstreams(id),
    ...
    def _cleanup_deleted_chunk(self):
        ...
        # We process one log stream at a time: this works well both
        # for a device holding a single long-lived stream with many
        # logs (we delete its logs gradually, batch by batch) and for
        # a device holding many streams with few logs each (each stream
        # is drained and removed right away, so we never scan a large
        # set of empty streams repeatedly).
        start = time()
        while time() - start < CLEANUP_DELETED_BUDGET:
            # pick a log stream of a deleted device
            rows = self.execute("""
                SELECT s.id
                FROM alllogstreams s
                JOIN alldevices d ON d.mac = s.issuer_mac
                WHERE d.type = 'deleted'
                LIMIT 1""")
            if len(rows) == 0:
                # no log stream left: just remove the remaining
                # 'deleted' rows (devices without any log stream)
                self.execute(
                    "DELETE FROM alldevices d WHERE d.type = 'deleted';")
                self.commit()
                return False
            # remove a bounded batch of logs of this stream
            stream_id = rows[0].id
            self.execute("""
                DELETE FROM alllogs
                WHERE ctid IN (
                    SELECT ctid FROM alllogs WHERE stream_id = %s LIMIT %s
                )""", (stream_id, CLEANUP_DELETED_BATCH))
            if self.c.rowcount < CLEANUP_DELETED_BATCH:
                # this stream has no more logs: remove it
                self.execute(
                    "DELETE FROM alllogstreams WHERE id = %s", (stream_id,))
            self.commit()
