...
class RegistryClientBase:
    ...
    def __init__(self, label, podman_url_prefix, login_host, protocol, auth,
    ...
    def pull(self, requester, server, image_fullname):
        ...
        if not url.startswith('docker.io') and not url.startswith(
                'docker-daemon:'):
    ...
    def login(self, requester):
        ...
        try:
            ...
            requester.stdout.write(
                f'Sorry, {self.label} registry login FAILED.\n')
            return False
        return True
...
class DockerDaemonClient(RegistryClientBase):
    ...
    async def async_images(self):
        ...
        for line in docker.image.ls('--format', '{{.Repository}} {{.Tag}}',
                                    '--filter', 'dangling=false', '--filter',
                                    'label=walt.node.models').splitlines():
...
class SkopeoRegistryClient(RegistryClientBase):
    ...
    async def async_get_labels(self, requester, fullname):
        ...
        if 'config' not in config:
            print('{fullname}: unknown image config format.'.format(
                fullname=fullname))
            return {}
...
class DockerHubClient(SkopeoRegistryClient):
    ...
    def __init__(self):
        super().__init__('hub', 'docker.io/', 'docker.io', 'https', 'basic',
                         ('pull', 'inspect', 'search'))
...
class DockerRegistryV2Client(SkopeoRegistryClient):
    ...
    async def async_multi_page_registry_v2_query(self, requester, url,
                                                 json_name):
        ...
        while True:
            ...
            if user_auth_pattern is not None:
                url = url.replace(f'{base_proto}://',
                                  f'{base_proto}://{user_auth_pattern}')
            json, links = await async_json_http_get(url,
                                                    return_links=True,
