...
class VPNManager:
    ...
    def enrollment(self, ip, pubkey):
        ...
        # if the node already had a vpnmac, error out
        if vpn_node.vpnmac is not None:
            return {"status": "KO",
                    "error_msg": "This board was already registered as a VPN node."}
        ...
        self.server.db.execute(
                """INSERT INTO vpnauth(vpnmac, pubkeycert, certid)
                   VALUES (%s, %s)""", (vpnmac, pubkeycert, cert_id))
        self.server.db.execute(
    ...
    def get_vpn_node_property(self, node_ip, prop_name):
        ...
        if vpn_node.vpnmac is None:
            return {"status": "KO", "error_msg": "This node is not enrolled yet."}
