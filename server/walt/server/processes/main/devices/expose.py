...
Failed: '{setting_value}' is not a valid value for option 'expose'.
        ...
        -> '80' (server port automatically chosen)
        -> '80[label=nginx]' (with a descriptive label)
        -> '80:8080[label=webapp],443:8443' (2 ports exposed)
        -> '80,443' (again, 2 ports exposed)
        -> or 'none'.
        We only accept labels with lowercase chars or dashes.
...
class ExposeManager:
    ...
    def parse_expose_setting_value(self, setting_value):
        rex_one_redirect = r"\d+(:\d+)?(\[label=[a-z][a-z-]*\])?"
        rex = f"{rex_one_redirect}(,{rex_one_redirect})*$"
        if setting_value != "none" and re.match(rex, setting_value) is None:
            return (False,)
        _explicit_redirects, _implicit_redirects = [], []
        if setting_value != "none":
            for redirect in setting_value.split(","):
                if '[' in redirect:
                    redirect, label_spec = redirect.split('[')
                    label = label_spec.split('=')[1][:-1]
                else:
                    label = None
                if ':' in redirect:
    ...
    def check_expose_setting_value(self, requester, devices_ip, setting_value):
        ...
        if len(implicit_redirects) > 0:
            all_device_ports = list(tuple(zip(*implicit_redirects))[0])
        else:
            all_device_ports = []
        if len(explicit_redirects) > 0:
            if len(devices_ip) > 1:
                requester.stderr.write(
                      ...
                      "several devices.\n")
                return False
            device_ports, server_ports, _ = tuple(zip(*explicit_redirects))
            ...
            for server_port in server_ports:
                if server_port >= self.dyn_ports_low_bound:
                    ...
                    return False
            for device_port, server_port, _ in explicit_redirects:
                ...
                if redir is None:
                    ...
                    # this port is currently in use for a redirect, check
                    # if this is for one of the devices we are currently
                    # reconfiguring, fail otherwise.
    ...
    def add_redirect(self, requester, server_port, device_ip,
                     device_port, label):
        check, err = self.check_bind_port(server_port)
        if check:
            redir = ExposeRedirect(self, server_port, device_ip,
                                   device_port, label)
            redir.start()
            self.redirects[server_port] = redir
        else:
            logline = (f"Failed to expose {device_ip}:{device_port} "
                       f"on server:{server_port} -- {err} (bypassing)\n")
            if requester is None:
                self.server.logs.platform_log("expose", line=logline,
                                              error=True)
            else:
    ...
    def choose_server_port(self, device_port, proposed_offset,
                           wrong_server_ports, old_server_ports):
        # 1st choice, check with the proposed offset if any
    ...
    def apply(self, requester, devices_ip, setting_value):
        ...
        old_redirects, new_redirects = set(), set()
        ...
        for dev_ip in devices_ip:
            for dev_p, serv_p, label in explicit_redirects:
                new_redirects.add((dev_ip, dev_p, serv_p, label))
                new_redirects_per_ip[dev_ip].append((dev_p, serv_p, label))
        ...
        for dev_ip in devices_ip:
            ...
            for dev_p, label in implicit_redirects:
        ...
        # stop old redirects
        for dev_ip, dev_p, serv_p, label in old_redirects - new_redirects:
            redir = self.redirects.pop(serv_p)
            redir.close()
        # create new redirects
        for dev_ip, dev_p, serv_p, label in new_redirects - old_redirects:
            self.add_redirect(requester, serv_p, dev_ip, dev_p, label)
        ...
        if setting_value == "none":
            ...
            for ip, ip_redirects in new_redirects_per_ip.items():
                redirect_options = []
                for dev_p, serv_p, label in sorted(ip_redirects):
                    option = f"{dev_p}:{serv_p}"
                    if label is not None:
                        option += f"[label={label}]"
                    redirect_options.append(option)
                result[ip] = ",".join(redirect_options)
        return result
    ...
    def restore(self):
        for row in self.server.db.execute("""\
            ...
            redirects = parsed[1]
            for device_port, server_port, label in redirects:
                self.add_redirect(None, server_port, row.ip,
                                  device_port, label)
    ...
    def get_web_links_info(self):
        ...
        for row in self.server.db.execute("""\
            ...
            assert parsed[0] is True
            dev_links = {}
            for dev_p, serv_p, label in parsed[1]:
                dev_links[dev_p] = (serv_p, label)
