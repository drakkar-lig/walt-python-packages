...
class NodeImageStore(object):
    ...
    def _wf_mount_images(self, update_wf, to_be_mounted, **env):
        if len(to_be_mounted) > 0:
            steps = []
            for img in to_be_mounted:
                step = functools.partial(self._wf_mount, img=img)
                steps.append(step)
    ...
    def _wf_mount(self, wf, img, **env):
        image_id, image_kib = img.image_id, img.size_kib
