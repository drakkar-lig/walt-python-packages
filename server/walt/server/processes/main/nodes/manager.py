...
Mode: {boot_mode} -- The OS diverges from original image.
Run 'walt help show boot-modes' & 'walt help show shells' for more info.
"""

MSG_NODE_CMD_VOLATILE = """\
Mode: {boot_mode} -- Node reboots discard changes outside /persist.
Run 'walt help show boot-modes' & 'walt help show shells' for more info.
"""
...
class NodesManager(object):
    ...
    def get_node_cmd_startup_msg(self, node_name):
        ...
        if len(node_info) != 1:
            return None
        boot_mode = node_info[0].boot_mode
        if boot_mode.endswith("-volatile"):
            msg = MSG_NODE_CMD_VOLATILE
        else:
