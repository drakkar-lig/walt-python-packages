...
def reboot_nodes(nodes, powersave, **env):
    ...
    wf = Workflow([
        wf_reboot_virtual_nodes, wf_soft_reboot_nodes, wf_hard_reboot_nodes,
        wf_reply_requester
    ], **env)
    # start process
    wf.run()
...
def wf_soft_reboot_nodes(wf, ev_loop, db, hard_only, remaining_nodes, **env):
    if hard_only or len(remaining_nodes) == 0:
        ...
        softrebootable_nodes = list(n for n in remaining_nodes
                                    if n.mac not in off_macs)
        node_request(ev_loop, softrebootable_nodes, 'REBOOT',
...
def wf_softreboot_callback(results, wf, remaining_nodes):
    ...
    wf.update_env(softrebooted=softrebooted,
...
def wf_filter_poe_rebootable(wf, nodes_manager, remaining_nodes, **env):
    hardreboot_errors = {}
    for node in remaining_nodes.copy():
        sw_info, sw_port = nodes_manager.devices.get_connectivity_info(
                                node.mac)
        if sw_info:
...
def wf_poe_reboot(wf, requester, db, blocking, remaining_nodes,
    ...
    wf.update_env(not_already_off=not_already_off,
                  already_off=already_off,
                  powered_off=[])
    if len(not_already_off) > 0:
        wf.insert_steps(
            [wf_poe_poweroff, wf_poe_after_poweroff,
             wf_poe_poweron, wf_poe_after_poweron])
    else:
        wf.insert_steps([wf_poe_poweron, wf_poe_after_poweron])
    wf.next()


def wf_poe_poweroff(wf, ev_loop, blocking, not_already_off,
                    hardreboot_errors, **env):
...
def wf_poe_poweron(wf, ev_loop, blocking, already_off, powered_off,
                   hardreboot_errors, **env):
...
def wf_reply_requester(wf, requester, task_callback, hard_only, vmrebooted,
                       softrebooted, hardrebooted, softreboot_errors,
                       hardreboot_errors, hard_reboot_method_name, **env):
    ...
    if len(hardreboot_errors) == 0:
        ...
        for errors, node_names in per_errors.items():
            if len(errors
                   ) == 1:  # no soft reboot error, just a poe reboot error
                hard_error = errors[0]
                explain = 'failed %s (%s)' % (hard_reboot_method_name,
                                              hard_error)
            else:
