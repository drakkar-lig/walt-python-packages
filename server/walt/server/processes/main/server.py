...
class Server(object):
    ...
    def register_fake_device(self, requester,
                 type="unknown", model=None, name=None):
        """register a fake device, useful in the test suite"""
        # random mac address generation
        mac = self.generate_free_mac("52:54:00")
        # request a free ip from dhcpd
        result = self.dhcpd.allocate_ip(mac)
        if result[0]:
            ip = result[1]
            self.add_or_update_device(
                    task=None, mac=mac, ip=ip, type=type, model=model,
                    name=name
            )
            return (mac, ip)
        else:
    ...
    def generate_vnode_info(self, requester):
        ...
        # request a free ip from dhcpd
        result = self.dhcpd.allocate_ip(free_mac)
        if result[0]:
            free_ip = result[1]
        else:
            requester.stderr.write("Failed: " + result[1])
    ...
    def create_vnode(self, requester, task, name):
        ...
        mac, ip, model = self.generate_vnode_info(requester)
        if ip is None:
            return False
