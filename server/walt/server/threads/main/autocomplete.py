...
def complete_device(server, partial_token):
    return tuple(dev.name for dev in server.db.select('devices')
...
def complete_set(possible_items, partial_token):
    ...
    possible = tuple(prefix + item for item in possible_items \
                                   if item.startswith(partial_item))
...
def complete_rescan_set_of_devices(server, partial_token):
    ...
    all_switches = list(dev.name for dev in server.db.select('devices') \
                                 if dev.type == 'switch')
...
def complete_cp_src(server, requester, username, entity_type, partial_token):
    ...
    for entity in possible_entities:
        if partial_token.startswith(f'{entity}:'):
            possible += fs_remote_completions(server, requester, entity_type, entity, partial_token)
        elif entity.startswith(partial_token):
            possible += (f'{entity}:',)
    return possible

def complete_cp_dst(server, requester, username, entity_type,
                    src_token, partial_dst_token):
    possible = ()
    ...
    if dst_is_remote:
        ...
        for entity in possible_entities:
            if partial_dst_token.startswith(f'{entity}:'):
                possible += fs_remote_completions(server, requester, entity_type, entity, partial_dst_token)
...
def complete_device_config_param(server, requester, argv):
    ...
    for device_name, device_conf in config_data.items():
        new_setting_names = set(name for name in device_conf['settings'].keys() \
...
def get_walt_clone_urls(server, username):
    # faster than using server.repositories.local
...
def get_docker_clone_urls(server, username):
    # this is a remote repository, so we do not filter out images of calling user
    return tuple(f'docker:{fullname}' for fullname in server.repositories.daemon.images())

def complete_image_clone_url(server, username, partial_token):
    repos = {
        'walt:': get_walt_clone_urls,
        'docker:': get_docker_clone_urls,
        'hub:': None    # too long for autocompletion, user has to use "walt image search"
    }
    if server.repositories.daemon is None:
        del repos['docker:']
...
def shell_autocomplete_switch(server, requester, username, argv):
    ...
    if arg_type == 'NODE':
        ...
        return complete_cp_dst(server, requester, username, 'node', prev_token, partial_token)
    elif arg_type == 'IMAGE_CP_SRC':
        return complete_cp_src(server, requester, username, 'image', partial_token)
    elif arg_type == 'IMAGE_CP_DST':
        return complete_cp_dst(server, requester, username, 'image', prev_token, partial_token)
...
def shell_autocomplete_process(server, requester, username, argv, debug):
    ...
    if len(possible) == 1:
        ...
        # to the image name...
        if arg_type in ('IMAGE_CP_SRC', 'IMAGE_CP_DST') and \
                partial_token.endswith(':') and \
                item == f'{partial_token}latest:':
        ...
        if arg_type in ('IMAGE', 'IMAGE_OR_DEFAULT') and \
                possible[1] == f'{possible[0]}:latest':
            possible = (possible[0],)
