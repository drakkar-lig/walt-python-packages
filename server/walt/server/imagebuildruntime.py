...
from walt.server.exttools import buildah, podman, findmnt
...
def get_session_id():
    return int(os.environ.get("WALT_BUILD_SESSION_ID"))
...
def get_bundle(cmd_args: list[str]) -> str:
    return Path(get_flag(cmd_args, "--bundle") or os.getcwd()).resolve()
...
@contextmanager
def get_container_state_path(container_id):
    with file_update_lock():
        yield TMP_DIR / (container_id + ".state")
...
def get_container_status(container_id):
    try:
        with get_container_state_path(container_id) as p:
            return json.loads(p.read_text())["status"]
    except FileNotFoundError:
        return "stopped"


def init_container_state(container_id, pid, bundle):
    state = {
        "ociVersion": "1.1.0",
        "id": container_id,
        "status": "created",
...
def write_run_on_node_script(rootfs, run_args, run_cwd, run_env):
...
def on_node_create(container_id, cmd_args: list[str]) -> None:
    ...
    bundle = get_bundle(cmd_args)
    init_container_state(container_id, pid, bundle)
...
def on_node_bg_process(container_id, cmd_args: list[str]) -> None:
    ...
    write_run_on_node_script(rootfs, run_args, run_cwd, run_env)
