...
def get_grub_boot_disk():
    try:
        p = subprocess.run('grub-probe -t disk /boot'.split(), capture_output=True, text=True)
        boot_device = p.stdout.strip()
        if not boot_device.startswith('/dev/mapper'):
            return boot_device
        boot_device_name = boot_device[len('/dev/mapper/'):]
        p = subprocess.run('lsblk --json --output name'.split(), capture_output=True, text=True)
