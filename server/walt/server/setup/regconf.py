...
def print_regconf_status(context, regconf):
    ...
    if len(regconf) == 0:
        print(s + highlight('incomplete') + \
                ' (tip: define at least one image registry)')
...
def select_reg_edit_menu(context, regconf, i, is_new):
    context['menu_info_function'] = \
        lambda context, regconf, valid: edit_reg_menu_info(context, regconf, i, is_new)
...
def edit_reg_menu_info(context, regconf, i, is_new):
    ...
    if 'label' in reg and 'host' in reg and 'port' in reg:
        # fully defined
        options.update({'Validate and return to main menu': (validate_registry_changes, i)})
...
def define_reg_property(context, regconf, i, k):
    ...
    while True:
        try:
            ...
            if k == 'label':
                value = prompt('Please enter a label for this registry:',
                         type=str)
                if len(re.sub("[a-z0-9-]", "", value)) > 0:
                    print('Sorry, only lowercase chars, digits, and dash ("-") are allowed.')
                    value = None
                elif value in ('hub', 'walt', 'docker'):
                    print(f'Value "{value}" is reserved.')
                    value = None
            elif k == 'description':
                value = prompt('Please enter a short description for this registry:',
                         type=str)
                if len(value) == 0:
                    print('Sorry, empty description is not allowed.')
                    value = None
                elif len(value) > MAX_DESC_LEN:
                    print(f'Sorry, the description should not exceed {MAX_DESC_LEN} chars.')
                    value = None
            elif k == 'host':
                value = prompt('Please indicate the IP or hostname of this registry:',
                         type=str)
...
def edit_regconf_interactive(regconf):
    ...
    print('Entering registries configuration editor... ', end=''); sys.stdout.flush()
    time.sleep(2)
    if regconf is None:
