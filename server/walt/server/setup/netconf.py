...
MESSAGE_NO_INTERFACE = framed('Note', '\n'.join(wrap("""\
...
MESSAGE_VIRTUAL_ONLY = framed('Note', '\n'.join(wrap("""\
...
def get_net_desc(netname):
    return { 'walt-net': 'platform network',
             'walt-adm': 'switches admin network'}.get(
...
def validate_ipv4_network(x):
    ...
    if net.prefixlen == 32:
        raise ValueError("The full network should be specified, not the single IP address.")
    return x
...
def wait_message_read():
    print(' '*71 + ']\r[', end=''); sys.stdout.flush()
    for i in range(70):
        print('*', end=''); sys.stdout.flush()
        time.sleep(0.28)
    print('\r' + ' '*72 + '\r', end=''); sys.stdout.flush()
...
def netname_row_values(netconf, netname):
    ...
    return (highlight(netname), net_desc, phys_intf,
            vlan, ipconf_type, ipconf)
...
def pretty_print_netconf(netconf):
    ...
    header = [ "network", "description", "phys. intf.", "vlan tagging",
               "ip mode", "ip conf" ]
    rows = []
    for netname in netnames:
        rows.append(netname_row_values(netconf, netname))
    print(framed('WalT network configuration',
...
def select_interface_edit_menu(context, netconf, netname):
    context['menu_info_function'] = \
        lambda context, netconf, valid: edit_interface_menu_info(context, netconf, netname)
...
def edit_interface_menu_info(context, netconf, netname):
    ...
    options.update({f'{netname} -- {verb} the physical interface':
                        (define_physical_interface, netname)})
    if has_phys_interface:
        if 'vlan' in netnameconf:
            options.update({
                f'{netname} -- disable VLAN tagging':
                (disable_vlan, netname)})
            options.update({
                f'{netname} -- change VLAN tag':
                (change_vlan, netname)})
        else:
             options.update({
                f'{netname} -- enable VLAN tagging':
                (change_vlan, netname)})
    if netname == 'walt-net' or has_phys_interface:
        verb = 'change' if 'ip' in netnameconf else 'define'
        options.update({
            f'{netname} -- edit IP configuration':
            (edit_ip, netname)})
    if netname == 'walt-adm':
        options.update({f'{netname} -- disable this optional network': (disable_walt_adm,)})
...
def edit_ip(context, netconf, netname):
    if netname == 'walt-net':
        print('The walt server fully manages this walt-net network and provides its own DHCP server.')
        print()
        print('The only change allowed here is the IP network subnet.')
        print('It may be useful to define a larger one if the platform has to grow with many devices.')
        print()
        ip_mode = 'static'
    else:
    ...
    if ip_mode == 'dhcp':
        ...
        try:
            ip_conf = prompt('Please enter IP configuration (e.g. 10.0.12.55/27):',
                     type=str, validator = validate_ipv4_network)
...
def edit_netconf_interactive(netconf):
    ...
    print('Detecting wired network interfaces... ', end=''); sys.stdout.flush()
    wired_interfaces = []
    notes = []
    ...
    print('Entering network configuration editor... ', end=''); sys.stdout.flush()
    time.sleep(2)
    if netconf is None:
