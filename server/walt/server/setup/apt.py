...
def fix_dpkg_options():
    print('Tuning package manager for non-interactive use... ', end=''); sys.stdout.flush()
    conf = DPKG_CONF_PATH
...
def upgrade_and_install_packages(packages, upgrade_packets = False):
    ...
    cache = get_apt_cache()
    print('Reloading package cache... ', end=''); sys.stdout.flush()
    cache.update()
    cache.open()
    print('done')
    modified = False
    ...
    if upgrade_packets:
        ...
        modified = True
    if modified:
        msg = 'Installing or upgrading OS packages' if upgrade_packets else 'Installing OS packages'
