...
def printed_walltime(recipe_info):
    ...
    elems = []
    for n, unit in zip((int(n) for n in recipe_info['walltime'].split(':')),
                       ('hour', 'minute', 'second')):
        if n == 1:
            elems.append('1 ' + unit)
        elif n > 1:
...
def printed_schedule(recipe_info):
    return 'G5K reservation schedule: ' + {
...
def printed_deployment_info(recipe_info):
    ...
    if server_site is None and \
       get_total_node_count(recipe_info) == 0:
