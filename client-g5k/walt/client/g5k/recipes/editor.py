...
def print_recipe_status(recipe_info):
    s = 'Recipe status: '
    undefined = []
    ...
    if valid:
        ...
        print(s + highlight('not available') + \
                ' (tip: ' + res_info['tip'] + ')')
    return valid

def define_walt_server(recipe_info):
    site = choose('On which site should WalT server run?', \
                  get_g5k_sites_for_walt_server())
    recipe_info['server']['site'] = site

def change_walt_nodes(recipe_info):
...
def edit_recipe(recipe_info):
    with alternate_screen_buffer():
        while True:
            ...
            valid = print_recipe_status(recipe_info)
            print()
            options = {
