...
In this case, users can fallback to the hybrid boot mode.
...
Important notes:
* The first time a node boots an image in hybrid mode, the bootup time will be quite long
  (several minutes longer) because of the complete copy of the WalT image to the local disk.
